...
public class ComponenteVenta implements Componente {
    ...
    public void recibirNotificacion(String evento, Map<String, Object> datos) {
        ...
        switch (evento) {
            case "ACTUALIZAR_INVENTARIO":
                ...
                if (datos.containsKey("producto_nombre") && datos.containsKey("stock_actual")) {
                    String productoNombre = (String) datos.get("producto_nombre");
                    int stockActual = (int) datos.get("stock_actual");
                    verificarDisponibilidadProducto(productoNombre, stockActual);
                }
                else if (datos.containsKey("producto_id") && datos.containsKey("stock_actual")) {
            ...
            case "SIMULACION_VENTA":
                // Evento especial para simulación
                LOGGER.log(Level.INFO, "Simulando procesamiento de venta");
                double total = 0.0;
                if (datos.containsKey("total")) {
                    total = (double) datos.get("total");
                }
                procesarSimulacionVenta(total);
                break;
        }
    }
    
    /**
     * Verifica si un producto tiene suficiente stock disponible (por nombre)
     */
    private void verificarDisponibilidadProducto(String productoNombre, int stockActual) {
        LOGGER.log(Level.INFO, "Verificando disponibilidad de producto {0}, stock actual: {1}", 
                new Object[]{productoNombre, stockActual});
    ...
    /**
     * Verifica si un producto tiene suficiente stock disponible (por ID)
     */
    private void verificarDisponibilidadProducto(int productoId, int stockActual) {
        LOGGER.log(Level.INFO, "Verificando disponibilidad de producto ID {0}, stock actual: {1}", 
                new Object[]{productoId, stockActual});
        
        if (stockActual <= 5) {
            LOGGER.log(Level.WARNING, "Producto ID {0} con stock bajo: {1} unidades", 
                    new Object[]{productoId, stockActual});
        }
    }
    
    /**
     * Procesa una simulación de venta
     */
    private void procesarSimulacionVenta(double total) {
        LOGGER.log(Level.INFO, "Procesando simulación de venta con total: ${0}", total);
    ...
    public void notificarNuevaVenta(Venta venta, DetalleVenta[] detalles) {
        if (mediador != null) {
            ...
            datos.put("usuario_id", venta.getUsuarioID());
    ...
    /**
     * Simulación simplificada de venta para demo
     */
    public void simularVenta(int productoId, int cantidad, double precio) {
        if (mediador != null) {
            Map<String, Object> datos = new HashMap<>();
            ...
            mediador.notificar(this, "NUEVA_VENTA", datos);
            LOGGER.log(Level.INFO, "Venta simulada: {0} unidades del producto {1} a ${2} cada una", 
                    new Object[]{cantidad, productoId, precio});
        }
    }
