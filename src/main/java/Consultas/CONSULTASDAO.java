...
public class CONSULTASDAO {
    ...
    public boolean completarVenta(int usuarioId, List<Producto> productos) {
        String sqlVenta = "INSERT INTO ventas (UsuarioID, FechaVenta, PrecioTotal) VALUES (?, NOW(), ?)";
        String sqlDetalleVenta = "INSERT INTO detallesventa (VentaID, ProductoID, Cantidad, PrecioUnitario, PrecioTotal) VALUES (?, ?, ?, ?, ?)";
    ...
    public boolean crearProd(String nombre, int areaID, double precio, int unidades, String fecha, int codB, String marca,
        ...
        try (PreparedStatement pstmt = con.prepareStatement(sql)) {
            ...
            pstmt.setObject(5, producto.getFechaCaducidad());
    ...
    public List<Producto> obtenerDetallesVenta(int ventaId) throws SQLException {
        ...
        String sql = "SELECT p.ProductoID, p.Nombre, p.Precio, dv.Cantidad, dv.PrecioTotal FROM productos p JOIN detallesventa dv ON p.ProductoID = dv.ProductoID WHERE dv.VentaID = ?";
